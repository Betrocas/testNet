...
function consultar() {
    fetch("https://localhost:7097/Product/Consultar")
        .then((resp) => resp.json())
        .then(data => {
            clienteLista = data;
            console.log(data);
...
function renderList(list) {
    ...
    list.forEach(e => {
        ...
        card.innerHTML = renderObj(e.id, e.nombre, e.precio, e.categoryId);
...
function renderObj(id, nombre, precio, categoria) {
    let obj = `
                <div class="card-body">
                    ...
                    <div>
                        <label class="form-label">Apellido</label>
                        <input type="text" class="form-control border-0" value="${precio}" id="apellido_${id}">
                    </div>
                    <div>
                        <label class="form-label">Edad</label>
                        <select class="form-select" id="category_${id}"></select>
                        <input type="text" class="form-control border-0" value="${categoria}" id="ca_${id}">
                    </div>
                    <div class="row mt-2">
                        <div class="col-6">
...
function renderClasificationList() {
